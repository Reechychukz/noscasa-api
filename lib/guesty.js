...
function getCacheKey(method, path, params) {
    return `${method}:https://open-api.guesty.com/v1${path}:${JSON.stringify(params || {})}`;
}
...
async function guesty(method, path, data = null, params = {}) {
  ...
  for (let attempt = 0; attempt <= MAX_RETRY_ATTEMPTS; attempt++) {
    let res;
    try {
      const url = new URL(`https://open-api.guesty.com/v1${path}`);
    ...
    console.log('Guesty response for', methodUpper, path, 'status', res.status, 'body:', bodyText);

    if (res.status < 200 || res.status >= 300) {
      const retryAfter = res.headers.get('retry-after');
      const error = new Error(`Guesty API returned status ${res.status}: ${bodyText}`);
      error.response = { status: res.status, data: bodyText };
      lastError = error;
    ...
    try {
      ...
      console.error('Failed to parse JSON from Guesty API:', bodyText);
      if (staleCache) {
...
async function getToken() {
  ...
  for (let attempt = 0; attempt <= MAX_RETRY_ATTEMPTS; attempt++) {
    try {
      ...
      const bodyText = await res.text();
      if (!res.ok) {
        const error = new Error(`Guesty token request failed with status ${res.status}: ${bodyText}`);
        error.response = { status: res.status, data: bodyText, headers: { 'retry-after': res.headers.get('retry-after') } };
        throw error;
      }

      const tokenData = JSON.parse(bodyText);
      _token = tokenData.access_token;
      _tokenExpiry = now + (tokenData.expires_in - 300) * 1000;

      // Cache in Redis
      if (client) {
        try {
          await client.setEx('guesty_token', Math.ceil((tokenData.expires_in - 300) / 1), JSON.stringify({ token: _token, expiry: _tokenExpiry }));
        } catch (err) {
